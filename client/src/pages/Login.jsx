...
const Login = () => {
  ...
  return (
    <div className='flex items-center justify-center min-h-screen px-6 sm:px-0 bg-gradient-to-br from-indigo-200 to-green-400'>
    <img onClick={() => navigate('/')} src={Logo} alt="" className='absolute w-28 sm:w-32 left-5 sm:left-20 top-5 cursor-pointer' />
    <div className='bg-slate-900 p-10 rounded-lg shadow-lg w-full sm:w-96 text-green-300 text-sm'>
      ...
      <form onSubmit={onSubmitHandler}>
        {
          state === 'Sign Up' && (
        <div 
        className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C] text-white'>
      ...
      {state === 'Sign Up' ? (
          ...
          <span onClick={() => setState('Login')} className='text-green-300 cursor-pointer underline'>Login here</span>
        </p>
      ) : (
        <p className='text-gray-400 text-center text-xs mt-4 '>Don't have an account?{' '}
        <span onClick={() => setState('Sign Up')} className='text-green-300 cursor-pointer underline'>Sign Up</span>
      </p>
      )}
